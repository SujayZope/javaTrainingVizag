...
public class LoginDaoImpl implements LoginDao {
	...
	public String validateMe(Login login) {
		...
		//search data in Pharmacy by OwnerId
		...
		if (loginFound != null) {
			...
			if (count == 1) {
				...
				if ((new Date()).after(ph.getExpireDate())) {
					...
					ph.setStatus(Status.valueOf("REWORK"));
					System.out.println("Status   " + ph.getStatus());
					session1.update(ph);
					trans1.commit();
...
//					}
		...
		//User login for first time using otp
	...
	public String sendOtp(String email, String userName) {
		...
		SendMail.sendInfo(email, "One Time Password(OTP) for Account recovery From Password Expiry process",
				"Your One Time Password(OTP) for Password Expiry recovery is " + otpNew);
		Login login = searchByUserName(userName);
		login.setOtp(otpNew);
